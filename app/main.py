...
async def lifespan(app: FastAPI):
    logger.info(
        """Starting cv-performance service
        ╔═══════════════════════════╗
        ║    cv-performance-svc     ║
        ╚═══════════════════════════╝"""
    )
...
app = FastAPI(
    docs_url=None, openapi_url=None, redoc_url=None, lifespan=lifespan, title=meta.title, version=settings.RELEASE_VER
)
...
@app.get("/", status_code=200, response_model=dict, summary="Get service information")
async def root():
    return {
...
@app.get("/exercises", status_code=200, response_model=dict, summary="List available exercise configurations")
...
@app.get(
    "/config/{exercise_name}", status_code=200, response_model=dict, summary="Get configuration for a specific exercise"
)
async def get_exercise_config(exercise_name: str):
    ...
    try:
        ...
        return {"exercise": exercise_name, "version": "2.0", "config": config}
    except Exception as e:
        return JSONResponse(
            status_code=404,
            content={"error": f"Exercise configuration not found: {exercise_name}", "details": str(e)},
        )
...
async def get_documentation():
    """API documentation"""
    return get_swagger_ui_html(openapi_url="/openapi.json", title=meta.title + " " + meta.metadata)


@app.get("/openapi.json", include_in_schema=False)
async def openapi():
    """OpenAPI schema"""
    return get_openapi(title=meta.title, version=settings.RELEASE_VER + " " + meta.metadata, routes=app.routes)
...
def main():
    ...
    try:
        ...
        performance_service.run()  # This blocks in main thread
